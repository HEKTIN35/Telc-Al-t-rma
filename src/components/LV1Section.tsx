...
export default function LV1Section({
  ...
  function makeBlank(id: string): ReactNode {
    ...
    return (
      <span
        ...
        className={cn(
          "mx-1 my-0.5 inline-flex cursor-pointer items-center gap-1 rounded-lg border border-dashed border-slate-500/80 bg-slate-800/55 px-2 py-0.5 align-baseline font-sans text-[0.7em] font-semibold whitespace-nowrap transition-all duration-150 select-none text-slate-100",
          filled && "border-solid border-violet-400/80 bg-violet-500/10 text-violet-100",
      ...
      >
        ...
        <button
          ...
          className="ml-0.5 rounded border border-slate-600 bg-slate-800 px-1 text-[0.85em] font-medium text-slate-200 hover:bg-slate-700"
        ...
        {filled && (
          <button
            ...
            className="rounded border border-slate-600 bg-slate-800 px-1 text-[0.85em] font-medium text-slate-200 hover:bg-rose-500/10 hover:text-rose-200"
            title="Kaldır"
          >×</button>
        )}
      </span>
  ...
  }, [data.text, answers, marks, dragOverId, popId]);
    ...
    >
      {splitView ? (
        ...
        >
          ...
          >
            <div className="reading-text">{nodes}</div>
          </div>
